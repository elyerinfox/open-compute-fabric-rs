//! Small helper for shelling out to host inventory/BMC tooling.
//!
//! Both the DMI collector (`dmidecode`) and the IPMI controller (`ipmitool`)
//! drive their real binary through [`run`]. It captures stdout/stderr and maps
//! any failure — including a missing binary — onto [`Error::Provider`] tagged
//! with the command name, so callers get a uniform error surface and never
//! panic. This mirrors `ocf-kernel`'s `exec::run`.
