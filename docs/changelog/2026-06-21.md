...
- **Relay bounce so *all* nodes can talk**: two `private` nodes route through a
  ...
  The relay is chosen by `pick_relay` (lowest-RTT **alive** relay, using the
  latency map), and WireGuard is **re-programmed on membership change** so a
  private node fails over to another live relay. The invariant: given ≥ 1 reachable
  relay, every pair of nodes can reach each other.
