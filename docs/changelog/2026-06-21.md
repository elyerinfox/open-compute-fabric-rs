...
- **Reachability-aware peering — reverse-connect for NAT'd nodes**: `plan_wg_peers`
  shapes each peer's endpoint/keepalive by reachability, so a node with **no public
  IPv4** joins by reverse-connecting. A `private` node pins endpoints + keepalive
  toward every `public`/`relay` peer (dials out, holds its NAT mapping open); those
  peers leave the private node's endpoint **unset (roam-learned)**. `set_peer`'s
  endpoint is now optional, and `GET /api/v1/fabric/wireguard` reports each peer's
  `reachability`, `endpoint` (`null` = roam-learned), and `keepalive`.
- **Relay bounce so *all* nodes can talk**: two `private` nodes route through a
  relay — the keepalive is the **held tunnel** each keeps open to the relay, the
  peer's `/32` goes into the relay's `allowed-ips`, and a **relay node enables IP
  forwarding** (`enable_ip_forwarding`) so the kernel actually forwards the bounce.
  WireGuard is **re-programmed on membership change** so a private node fails over
  to another live relay. The invariant: given ≥ 1 reachable relay, every pair of
  nodes can reach each other.
- **Graph-aware multi-relay routing**: a `RouteGraph` (nodes + reachability,
  RTT-weighted edges, Dijkstra) computes the next-hop relay **per destination**
  over a fleet-wide latency view the nodes gossip (a `latency` control verb +
  prober fetch) — so different destinations use different relays, each chosen by
  the full `self → relay → dest` cost, and multi-hop chains fall out. This drives
  the WireGuard `via` per peer (replacing the single-relay pick) and
  `GET /api/v1/fabric/routes` (now showing the full `hops` path).
...
- **Graph-aware routing.** `RouteGraph` (`routing.rs`) models the fabric as a
  weighted graph (nodes + reachability, edges between any non-both-private pair,
  RTT weights) and runs Dijkstra; `next_relay(src, dst)` is the next-hop relay on
  the shortest path. Because the cost is the whole `self → relay → dst` sum over a
  fleet-wide RTT view the nodes **gossip** (each fetches peers' latency maps via a
  `latency` control verb), **different destinations pick different relays** and
  multi-hop chains fall out — this superseded the earlier single-relay `plan_route`.
...
```mermaid
flowchart TD
    A[RouteGraph.path self to dst] --> B{edge self–dst?}
    B -->|yes| C[Direct]
    B -->|no| D[Dijkstra: min self → relay → dst]
    D --> E{path?}
...
| `crates/ocf-fabric/src/routing.rs` | `RouteGraph` — graph-aware shortest-path / next-hop relay routing |
...
- New focused tests today include: streaming round-trip (5 MB, multi-record) and
  ...
  measure-and-record over a loopback ping server; the `RouteGraph` cases (direct,
  via-relay, lowest-total-cost relay, different-relays-per-destination,
  unreachable) and the reachability-aware peer planner; the relay
