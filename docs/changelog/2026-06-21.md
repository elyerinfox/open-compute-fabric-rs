...
- **Reachability-aware peering — reverse-connect for NAT'd nodes**: `plan_wg_peers`
  ...
  peers leave the private node's endpoint **unset (roam-learned)**. Two private
  nodes route through a `relay`'s `allowed-ips`. `set_peer`'s endpoint is now
  optional, and `GET /api/v1/fabric/wireguard` reports each peer's `reachability`,
  `endpoint` (`null` = roam-learned), and `keepalive`.
