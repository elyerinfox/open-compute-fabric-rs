...
| Date | Highlights |
|------|-----------|
| [2026-06-22](2026-06-22.md) | Multi-node Raft over the encrypted fabric (cross-host cluster formation from seeds) and split-brain mitigation (quorum on writes + a quorum-gated failure detector); a package-update handler, security-update warnings, and OSV vulnerability scanning; automatic node identity + LAN address; **RBAC enforcement** gating key API functionality |
